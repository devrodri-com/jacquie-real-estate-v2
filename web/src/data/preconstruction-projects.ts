...
const visiblePreconstructionDetailIds = new Set([
  "2200-brickell",
  "elle-residences",
  "midtown-park",
]);
...
function hasDetailSlug(
  ...
  return Boolean(project.detailSlug);
}

export function getVisiblePreconstructionProjects(): VisiblePreconstructionProject[] {
  return preconstructionProjects.filter(
    (project) =>
      visiblePreconstructionDetailIds.has(project.id) && project.detailSlug,
  ).filter(hasDetailSlug);
}
